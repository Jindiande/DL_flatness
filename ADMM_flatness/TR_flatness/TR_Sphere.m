...
function q = TR_Sphere( Y, mu, q_init,q_exist )% q_exist is former result n*k,q_init is n*1
	[n,~] = size(Y);
	...
	%% Main Iteration
	for iter = 1:MaxIter
        if(size(q_exist,2)==(size(q_exist,1)-1))
           U = null(q')
        else
           U = null([q,q_exist]'); % basis of the tangent space T_q = { v| v'*q =0}
		...
		%solve TR-subprolbem
		A = U' * H * U - q' * g * eye(size(U,2)); % hessian in the tangent space T_q
        ...
        %fprintf('Iter = %d, Obj = %f, TR Size = %f, Step = %f, rho = %f \n',iter,f, Delta,t,rho);
